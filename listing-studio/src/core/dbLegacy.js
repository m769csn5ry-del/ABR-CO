/* Couche « base de données » de l'atelier d'annonces (index.html).
 *
 * Elle porte le schéma de la première version : un espace de travail par
 * utilisateur, des projets d'annonces, des photos. La console d'exploitation
 * s'appuie sur `core/db.js`, qui décrit un modèle multi-organisation différent
 * (dossiers, contrats, commissions) dans un espace de stockage distinct.
 *
 * Les deux coexistent volontairement : l'atelier reste utilisable tel quel, et
 * `db.migrateFromV1()` importe ces données dans le nouveau modèle à la
 * première ouverture de la console.
 *
 * Isolation : chaque enregistrement porte un `userId`. Toute lecture passe par
 * le dépôt, qui filtre sur l'utilisateur courant.
 */
